# Bootstrappable MicroBitcoin Core Builds

This directory contains the files necessary to perform bootstrappable MicroBitcoin
Core builds.
...
To build MicroBitcoin Core reproducibly with all default options, invoke the
...
2. The `guix-codesign` command ***requires*** a `DETACHED_SIGS_REPO` flag.
    * _**DETACHED_SIGS_REPO**_
      ...
      MicroBitcoin Core version being built.
...
```
env DETACHED_SIGS_REPO=<path/to/micro-detached-sigs> ./contrib/guix/guix-codesign
```
...
repository](https://github.com/micro-core/guix.sigs).
