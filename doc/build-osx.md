...
This guide describes how to build microd, command-line utilities, and GUI on macOS
...
These tools must be installed in order to build MicroBitcoin Core from source.
...
### 4. Clone MicroBitcoin repository

`git` should already be installed by default on your system.
Now that all the required dependencies are installed, let's clone the MicroBitcoin Core repository to a directory.
...
It is not necessary to build wallet functionality to run `microd` or  `micro-qt`.
To enable legacy wallets, you must install `berkeley-db@4`.
To enable [descriptor wallets](https://github.com/MicroBitcoinOrg/MicroBitcoin/blob/master/doc/descriptors.md), `sqlite` is required.
...
MicroBitcoin Core includes a GUI built with the cross-platform Qt Framework.
...
See the notes in [#7714](https://github.com/MicroBitcoinOrg/MicroBitcoin/issues/7714).
...
You can deploy a `.dmg` containing the MicroBitcoin Core application using `make deploy`.
...
## Building MicroBitcoin Core

### 1. Configuration

There are many ways to configure MicroBitcoin Core, here are a few common examples:
...
Run the following in your terminal to compile MicroBitcoin Core:
...
## Running MicroBitcoin Core

MicroBitcoin Core should now be available at `./src/microd`.
If you compiled support for the GUI, it should be available at `./src/qt/micro-qt`.

The first time you run `microd` or `micro-qt`, it will start downloading the blockchain.
This process could take many hours, or even days on slower than average systems.

By default, blockchain and wallet data files will be stored in:

``` bash
/Users/${USER}/Library/Application Support/MicroBitcoin/
```

Before running, you may create an empty configuration file:

```shell
mkdir -p "/Users/${USER}/Library/Application Support/MicroBitcoin"

touch "/Users/${USER}/Library/Application Support/MicroBitcoin/micro.conf"

chmod 600 "/Users/${USER}/Library/Application Support/MicroBitcoin/micro.conf"
```
...
```shell
./src/microd -daemon      # Starts the micro daemon.
./src/micro-cli --help    # Outputs a list of command-line options.
./src/micro-cli help      # Outputs a list of RPC commands when the daemon is running.
./src/qt/micro-qt -server # Starts the micro-qt server mode, allows micro-cli control
```
