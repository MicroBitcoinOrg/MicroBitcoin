# Fuzzing MicroBitcoin Core using libFuzzer

## Quickstart guide

To quickly get started fuzzing MicroBitcoin Core using [libFuzzer](https://llvm.org/docs/LibFuzzer.html):
...
# libFuzzer" on https://github.com/MicroBitcoinOrg/MicroBitcoin/blob/master/doc/fuzzing.md#macos-hints-for-libfuzzer
$ make
...
[`process_message`](https://github.com/MicroBitcoinOrg/MicroBitcoin/blob/master/src/test/fuzz/process_message.cpp) is a fuzzing harness for the [`ProcessMessage(...)` function (`net_processing`)](https://github.com/MicroBitcoinOrg/MicroBitcoin/blob/master/src/net_processing.cpp). The available fuzzing harnesses are found in [`src/test/fuzz/`](https://github.com/MicroBitcoinOrg/MicroBitcoin/tree/master/src/test/fuzz).
...
The project's collection of seed corpora is found in the [`micro-core/qa-assets`](https://github.com/micro-core/qa-assets) repo.

To fuzz `process_message` using the [`micro-core/qa-assets`](https://github.com/micro-core/qa-assets) seed corpus:

```sh
$ git clone https://github.com/micro-core/qa-assets
...
If you find coverage increasing inputs when fuzzing you are highly encouraged to submit them for inclusion in the [`micro-core/qa-assets`](https://github.com/micro-core/qa-assets) repo.

Every single pull request submitted against the MicroBitcoin Core repo is automatically tested against all inputs in the [`micro-core/qa-assets`](https://github.com/micro-core/qa-assets) repo. Contributing new coverage increasing inputs is an easy way to help make MicroBitcoin Core more robust.
...
with certain assembly code from MicroBitcoin Core's code. See [developer notes on sanitizers](https://github.com/MicroBitcoinOrg/MicroBitcoin/blob/master/doc/developer-notes.md#sanitizers)
...
# Fuzzing MicroBitcoin Core using afl++

## Quickstart guide

To quickly get started fuzzing MicroBitcoin Core using [afl++](https://github.com/AFLplusplus/AFLplusplus):
...
# Fuzzing MicroBitcoin Core using Honggfuzz

## Quickstart guide

To quickly get started fuzzing MicroBitcoin Core using [Honggfuzz](https://github.com/google/honggfuzz):
...
## Fuzzing the MicroBitcoin Core P2P layer using Honggfuzz NetDriver

Honggfuzz NetDriver allows for very easy fuzzing of TCP servers such as MicroBitcoin
Core without having to write any custom fuzzing harness. The `microd` server
...
```sh
$ mkdir micro-honggfuzz-p2p/
$ cd micro-honggfuzz-p2p/
$ git clone https://github.com/MicroBitcoinOrg/MicroBitcoin
$ cd micro/
$ ./autogen.sh
$ git clone https://github.com/google/honggfuzz
...
$ git apply << "EOF"
diff --git a/src/microd.cpp b/src/microd.cpp
...
$ honggfuzz/honggfuzz --exit_upon_crash --quiet --timeout 4 -n 1 -Q \
      -E HFND_TCP_PORT=18444 -f inputs/ -- \
          src/microd -regtest -discover=0 -dns=0 -dnsseed=0 -listenonion=0 \
...
MicroBitcoin Core participates in Google's [OSS-Fuzz](https://github.com/google/oss-fuzz/tree/master/projects/micro-core)
program, which includes a dashboard of [publicly disclosed vulnerabilities](https://bugs.chromium.org/p/oss-fuzz/issues/list?q=micro-core).
...
because MicroBitcoin is a live P2P network, and not just standalone local software,
...
OSS-Fuzz also produces [a fuzzing coverage report](https://oss-fuzz.com/coverage-report/job/libfuzzer_asan_micro-core/latest).
