...
The MicroBitcoin Core daemon can be configured to act as a trusted "border
router", implementing the micro wire protocol and relay, making
...
The ZeroMQ feature in MicroBitcoin Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building microd:
...
For instance:
    ...
    $ microd -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubrawtx=ipc:///tmp/microd.tx.raw \
...
These options can also be provided in micro.conf.
...
From the perspective of microd, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into microd directly. Furthermore, no information is
...
using. MicroBitcoind appends an up-counting sequence number to each
