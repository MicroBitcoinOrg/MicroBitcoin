...
static void SetupMicroBitcoinUtilArgs(ArgsManager &argsman)
{
...
{
    SetupMicroBitcoinUtilArgs(args);
    std::string error;
    ...
    if (HelpRequested(args) || args.IsArgSet("-version")) {
        ...
        std::string strUsage = PACKAGE_NAME " micro-util utility version " + FormatFullVersion() + "\n";
        if (!args.IsArgSet("-version")) {
            strUsage += "\n"
                "Usage:  micro-util [options] [commands]  Do stuff\n";
            strUsage += "\n" + args.GetHelpMessage();
        }
...
static void grind_task(uint32_t nBits, CBlockHeader& header_orig, uint32_t offset, uint32_t step, std::atomic<bool>& found)
{
    ...
    while (!found && header.nNonce < finish) {
        ...
        do {
            if (UintToArith256(header.GetWorkHash()) <= target) {
