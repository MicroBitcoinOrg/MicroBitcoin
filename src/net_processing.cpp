...
bool static ProcessHeadersMessage(CNode *pfrom, CConnman *connman, const std::vector<CBlockHeader>& headers, const CChainParams& chainparams, bool punish_duplicate_invalid)
{
    ...
    if (!ProcessNewBlockHeaders(headers, state, chainparams, &pindexLast, &first_invalid_header)) {
        ...
        if (state.IsInvalid(nDoS)) {
            ...
            if (punish_duplicate_invalid && LookupBlockIndex(first_invalid_header.GetIndexHash())) {
...
bool static ProcessMessage(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv, int64_t nTimeReceived, const CChainParams& chainparams, CConnman* connman, const std::atomic<bool>& interruptMsgProc, bool enable_bip61)
{
    ...
    else if (strCommand == NetMsgType::CMPCTBLOCK && !fImporting && !fReindex) // Ignore blocks received while importing
    {
        ...
        if (!LookupBlockIndex(cmpctblock.header.GetIndexHash())) {
            received_new_header = true;
        }
        }
        ...
        if (fBlockReconstructed) {
            ...
            if (fNewBlock) {
                ...
                LOCK(cs_main);
                mapBlockSource.erase(pblock->GetIndexHash());
            }
            ...
            if (pindex->IsValid(BLOCK_VALID_TRANSACTIONS)) {
                ...
                MarkBlockAsReceived(pblock->GetIndexHash());
            }
        }
