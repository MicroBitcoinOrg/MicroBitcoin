This directory contains the source code for the MicroBitcoin graphical user interface (GUI). It uses the [Qt](https://www1.qt.io/developers/) cross-platform framework.
...
- Represents the main window of the MicroBitcoin UI.
...
- ClientModel is used by the main application `microgui` and several models like `peertablemodel`.
...
- (Deprecated) Used to process BIP21 payment URI requests. Also handles URI-based application switching (e.g. when following a micro:... link from a browser).
...
* UI elements like MicroBitcoinAmountField, which inherit from QWidget.
* `microstrings.cpp`: automatically generated
* `microunits.(h/cpp)`: MBC / mMBC / etc. handling
* `callback.h`
...
**Note:** Do not change `local/micro_en.ts`. It is updated [automatically](/doc/translation_process.md#writing-code-with-translations).
...
4. Enter `micro-qt` as the Project Name and enter the absolute path to `src/qt` as Location
...
8. While in the `Projects` tab, ensure that you have the `micro-qt` executable specified under `Run`
 - If the executable is not specified: click `"Choose..."`, navigate to `src/qt`, and select `micro-qt`
9. You're all set! Start developing, building, and debugging the MicroBitcoin GUI
