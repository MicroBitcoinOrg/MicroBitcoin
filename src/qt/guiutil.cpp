...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setValidator(new MicroBitcoinAddressEntryValidator(parent));
    widget->setCheckValidator(new MicroBitcoinAddressCheckValidator(parent));
}
...
bool parseMicroBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no micro: URI
    if(!uri.isValid() || uri.scheme() != QString("micro"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!MicroBitcoinUnits::parse(MicroBitcoinUnits::MBC, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parseMicroBitcoinURI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parseMicroBitcoinURI(uriInstance, out);
}

QString formatMicroBitcoinURI(const SendCoinsRecipient &info)
{
    bool bech_32 = info.address.startsWith(QString::fromStdString(Params().Bech32HRP() + "1"));

    QString ret = QString("micro:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(MicroBitcoinUnits::format(MicroBitcoinUnits::MBC, info.amount, false, MicroBitcoinUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
bool openMicroBitcoinConf()
{
    fs::path pathConfig = GetConfigFile(gArgs.GetArg("-conf", MICRO_CONF_FILENAME));
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "MicroBitcoin.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "MicroBitcoin (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("MicroBitcoin (%s).lnk", chain);
}

bool GetStartOnSystemStartup()
{
    // check for MicroBitcoin*.lnk
...
fs::path static GetAutostartFilePath()
{
    std::string chain = gArgs.GetChainName();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "micro.desktop";
    return GetAutostartDir() / strprintf("micro-%s.desktop", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = gArgs.GetChainName();
        // Write a micro.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=MicroBitcoin\n";
        else
            optionFile << strprintf("Name=MicroBitcoin (%s)\n", chain);
