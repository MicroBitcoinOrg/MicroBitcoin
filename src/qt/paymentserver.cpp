...
const QString MICRO_IPC_PREFIX("micro:");
...
void PaymentServer::ipcParseCommandLine(int argc, char* argv[])
{
    for (int i = 1; i < argc; i++)
    {
        ...
        // If the micro: URI contains a payment request, we are not able to detect the
        // network as that would require fetching and parsing the payment request.
        // That means clicking such an URI which contains a testnet payment request
        // will start a mainnet instance and throw a "wrong network" error.
        if (arg.startsWith(MICRO_IPC_PREFIX, Qt::CaseInsensitive)) // micro: URI
        {
            if (savedPaymentRequests.contains(arg)) continue;
            savedPaymentRequests.insert(arg);

            SendCoinsRecipient r;
            if (GUIUtil::parseMicroBitcoinURI(arg, &r) && !r.address.isEmpty())
            {
    ...
    optionsModel(nullptr)
{
    ...
    // on Mac: sent when you click micro: links
    ...
    if (startLocalServer)
    {
        ...
        if (!uriServer->listen(name)) {
            ...
            QMessageBox::critical(nullptr, tr("Payment request error"),
                tr("Cannot start micro: click-to-pay handler"));
        }
        else {
...
//
// OSX-specific way of handling micro: URIs
//
...
void PaymentServer::handleURIOrFile(const QString& s)
{
    ...
    if (s.startsWith("micro://", Qt::CaseInsensitive))
    {
        Q_EMIT message(tr("URI handling"), tr("'micro://' is not a valid URI. Use 'micro:' instead."),
            CClientUIInterface::MSG_ERROR);
    }
    else if (s.startsWith(MICRO_IPC_PREFIX, Qt::CaseInsensitive)) // micro: URI
    {
        QUrlQuery uri((QUrl(s)));
        // normal URI
        {
            SendCoinsRecipient recipient;
            if (GUIUtil::parseMicroBitcoinURI(s, &recipient))
            {
