...
uint256 SendCoins(CWallet& wallet, SendCoinsDialog& sendCoinsDialog, const CTxDestination& address, CAmount amount, bool rbf)
{
    ...
    entry->findChild<MicroBitcoinAmountField*>("payAmount")->setValue(amount);
...
//
//     QT_QPA_PLATFORM=xcb     src/qt/test/test_micro-qt  # Linux
//     QT_QPA_PLATFORM=windows src/qt/test/test_micro-qt  # Windows
//     QT_QPA_PLATFORM=cocoa   src/qt/test/test_micro-qt  # macOS
void TestGUI(interfaces::Node& node)
{
...

    {
        ...
        QString balanceComparison = MicroBitcoinUnits::formatWithUnit(unit, balance, false, MicroBitcoinUnits::SeparatorStyle::ALWAYS);
    ...
    // Amount input
    MicroBitcoinAmountField* amountInput = receiveCoinsDialog.findChild<MicroBitcoinAmountField*>("reqAmount");
...
void WalletTests::walletTests()
{
    ...
    if (QApplication::platformName() == "minimal") {
        ...
        QWARN("Skipping WalletTests on mac build with 'minimal' platform set due to Qt bugs. To run AppTests, invoke "
              "with 'QT_QPA_PLATFORM=cocoa test_micro-qt' on mac, or else use a linux or windows build.");
        return;
    }
#endif
    TestGUI(m_node);
