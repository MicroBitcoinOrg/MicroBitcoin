...
protected:
  struct Socket {
    socket_t sock = HTTP_INVALID_SOCKET;
#ifdef CPPHTTPLIB_OPENSSL_SUPPORT
    SSL *ssl = nullptr;
#endif

    bool is_open() const { return sock != HTTP_INVALID_SOCKET; }
  };
...
inline Server::Server()
    : new_task_queue(
      ...
      svr_sock_(HTTP_INVALID_SOCKET), is_running_(false) {
#ifndef _WIN32
...
inline void Server::stop() {
  if (is_running_) {
    assert(svr_sock_ != HTTP_INVALID_SOCKET);
    std::atomic<socket_t> sock(svr_sock_.exchange(HTTP_INVALID_SOCKET));
...
Server::write_content_with_provider(Stream &strm, const Request &req,
  ...
  auto is_shutting_down = [this]() {
    return this->svr_sock_ == HTTP_INVALID_SOCKET;
  };
...
inline int Server::bind_internal(const char *host, int port, int socket_flags) {
  ...
  if (svr_sock_ == HTTP_INVALID_SOCKET) { return -1; }
...
inline bool Server::listen_internal() {
...

  {
    ...
    while (svr_sock_ != HTTP_INVALID_SOCKET) {
#ifndef _WIN32
      ...
      if (sock == HTTP_INVALID_SOCKET) {
        ...
        if (svr_sock_ != HTTP_INVALID_SOCKET) {
...
inline bool ClientImpl::create_and_connect_socket(Socket &socket,
  ...
  if (sock == HTTP_INVALID_SOCKET) { return false; }
  socket.sock = sock;
  return true;
}
...
inline void ClientImpl::shutdown_socket(Socket &socket) {
  if (socket.sock == HTTP_INVALID_SOCKET) { return; }
...
inline void ClientImpl::close_socket(Socket &socket) {
  ...
  socket.sock = HTTP_INVALID_SOCKET;
}
...
inline void SSLClient::shutdown_ssl_impl(Socket &socket,
                                         bool shutdown_gracefully) {
  if (socket.sock == HTTP_INVALID_SOCKET) {
    assert(socket.ssl == nullptr);
    return;
  }
  if (socket.ssl) {
