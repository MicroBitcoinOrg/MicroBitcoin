                              ...
                              bool bypass_limits, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache, bool test_accept)
{
...

    {
        ...
        for (CTxMemPool::txiter it : allConflicting)
        {
            LogPrint(BCLog::MEMPOOL, "replacing tx %s with %s for %s MBC additional fees, %d delta bytes\n",
...
bool ReadBlockFromDisk(CBlock& block, const CDiskBlockPos& pos, const Consensus::Params& consensusParams)
{
    ...
    // Check the header
    if (!CheckProofOfWork(block.GetWorkHashCached(), block.nBits, consensusParams)) {
...
bool ReadBlockFromDisk(CBlock& block, const CBlockIndex* pindex, const Consensus::Params& consensusParams)
{
    ...
    if (!ReadBlockFromDisk(block, blockPos, consensusParams))
        return false;
    if (block.GetIndexHash() != pindex->GetBlockHash())
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    if (block.GetIndexHash() != chainparams.GetConsensus().hashGenesisBlock) {
    ...
    if (block.vtx[0]->GetValueOut() > blockReward && block.GetIndexHash() != chainparams.GetConsensus().hashGenesisBlock)
    ...
    do {
...

        {
            ...
            do {
                ...
                if (!ActivateBestChainStep(state, chainparams, pindexMostWork, pblock && pblock->GetIndexHash() == pindexMostWork->GetBlockHash() ? pblock : nullBlockPtr, fInvalidFound, connectTrace))
                    return false;
